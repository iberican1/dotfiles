...
# Define a function to restore the screen
restore_screen() {
    tput reset
}
...
execute_action() {
    ...
    sleep 2  # Simulate the action taking some time
...
actionA() {
    ...
    #read response
    #execute_action bash $HOME/bin/pieMusic > /dev/null 2>&1
    clear_screen
    echo "Running action $1..."
    #sleep 2  # Simulate the action taking some time
    bash $HOME/bin/pieMusic > /dev/null 2>&1
...
actionJ() {
    ...
    google-chrome-stable http://192.168.2.12:4747/
    echo -n "Press enter to continue ... "
    read response
...
menuItems=(
    ...
    "8. Youtube Download Script - copy the link from the above and fire away!"
